...
exports.deleteComment = exports.createComment = exports.getComments = exports.getPostDetail = exports.getHomeFeed = exports.getCollectionPosts = exports.getUserCollections = exports.getTaggedPosts = exports.getUserPosts = exports.unbookmarkPost = exports.bookmarkPost = exports.unlikePost = exports.likePost = exports.createCollection = exports.getPopularTags = exports.getMyCollections = exports.searchFollowingFriends = exports.deletePost = exports.updatePost = exports.createPost = void 0;
const db_1 = require("../config/db");
const jsonHelpers_1 = require("../utils/jsonHelpers");
// 커서 유틸: "<iso>|<id>" 형식 사용
const buildCursorCondition = (cursor) => {
    if (!cursor)
        return {};
...
const makeNextCursor = (post) => `${post.createdAt.toISOString()}|${post.id}`;
// 게시글 생성
const createPost = async (req, res, next) => {
    try {
        ...
        const { title, description, tags, collection, taggedFriends, isPrivate, images, effect, } = req.body;
        // 필수 필드 검증
        if (!title ||
            !images?.original ||
            !images?.background ||
        ...
        if (collection) {
            const existingCollection = await db_1.prisma.collection.findFirst({
                where: {
        ...
        const result = await db_1.prisma.$transaction(async (tx) => {
            ...
            const post = await tx.post.create({
                data: {
                    ...
                    effect: effect ? (0, jsonHelpers_1.toPrismaJson)(effect) : null,
                },
            });
            // 2. 이미지 생성
        ...
        const createdPost = await db_1.prisma.post.findUnique({
            where: { id: result.id },
            include: {
                author: {
                    select: {
...
const updatePost = async (req, res, next) => {
    try {
        ...
        // 게시글 확인 및 권한 체크
        const existing = await db_1.prisma.post.findUnique({ where: { id: postIdNum } });
        ...
        const { title, description, isPrivate, collection, tags } = req.body;
        // 컬렉션 체크
        ...
        if (collection !== undefined) {
            if (collection === null || collection === "") {
                ...
                const col = await db_1.prisma.collection.findFirst({
        ...
        // 수정된 게시글 조회 및 응답
        const updated = await db_1.prisma.post.findUnique({
            where: { id: postIdNum },
            include: {
        ...
        const response = {
            ...
            taggedFriends: updated.taggedFriends?.map((pf) => ({
                id: pf.user.id,
                name: pf.user.name,
                profileImageUrl: pf.user.profileImageUrl || undefined,
            })) || [],
...
const deletePost = async (req, res, next) => {
    try {
        ...
        const post = await db_1.prisma.post.findUnique({ where: { id: postIdNum } });
...
// 팔로워 친구 검색
const searchFollowingFriends = async (req, res, next) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ message: "로그인이 필요합니다" });
        ...
        const limitNum = Math.min(parseInt(limit) || 10, 50); // 최대 50명
        // 팔로잉 중인 사용자들 중에서 검색
...
exports.searchFollowingFriends = searchFollowingFriends;
// 내 컬렉션 목록 조회
const getMyCollections = async (req, res, next) => {
    try {
        ...
        const collections = await db_1.prisma.collection.findMany({
            where: {
                ownerId: userId,
            },
            include: {
...
const getPopularTags = async (req, res, next) => {
    try {
        ...
        const popularTags = await db_1.prisma.tag.findMany({
            include: {
                _count: {
                    select: {
                        posts: true,
...
const likePost = async (req, res, next) => {
    try {
        ...
        await db_1.prisma.postLike.create({ data: { userId, postId: postIdNum } });
...
const unlikePost = async (req, res, next) => {
    try {
        ...
        await db_1.prisma.postLike.deleteMany({ where: { userId, postId: postIdNum } });
        return res
            .status(200)
...
const bookmarkPost = async (req, res, next) => {
    try {
        ...
        const existing = await db_1.prisma.bookmark.findUnique({
            where: { userId_postId: { userId, postId: postIdNum } },
        });
        if (existing) {
            return res
        ...
        await db_1.prisma.bookmark.create({ data: { userId, postId: postIdNum } });
        return res
            .status(201)
...
const unbookmarkPost = async (req, res, next) => {
    try {
        ...
        await db_1.prisma.bookmark.deleteMany({ where: { userId, postId: postIdNum } });
        return res
            .status(200)
    ...
    try {
        ...
        if (viewerId && posts.length > 0) {
            ...
            const [likes, bookmarks] = await Promise.all([
                db_1.prisma.postLike.findMany({
                    where: { userId: viewerId, postId: { in: ids } },
                    select: { postId: true },
                }),
                db_1.prisma.bookmark.findMany({
                    where: { userId: viewerId, postId: { in: ids } },
                    select: { postId: true },
                }),
            ]);
        ...
        const results = items.map((post) => ({
            ...
            taggedFriends: post.taggedFriends?.map((pf) => ({
                id: pf.user.id,
                name: pf.user.name,
                profileImageUrl: pf.user.profileImageUrl || undefined,
            })) || [],
...
const getCollectionPosts = async (req, res, next) => {
    try {
        ...
        // 컬렉션 존재/권한 확인
        const collection = await db_1.prisma.collection.findUnique({
            where: { id: colId },
        ...
        const cursorCondition = cursor ? { id: { lt: parseInt(cursor) } } : {};
        // 게시글 공개 정책
        const postPrivacyFilter = collection.ownerId === viewerId
            ? {}
            : {
                OR: [
                    { isPrivate: false },
...
exports.getCollectionPosts = getCollectionPosts;
// 사용자의 태그 선호도 분석
const getUserTagPreferences = async (userId) => {
    // 사용자가 북마크한 게시글들의 태그를 분석하여 선호도 계산
    const bookmarkedTags = await db_1.prisma.tag.findMany({
        where: {
            posts: {
                some: {
                    post: {
...
// 게시글 알고리즘 점수 계산
const calculatePostScore = (post, userId, isFollowing, tagPreferences, weights) => {
    let score = 0;
    // 팔로잉 가중치
    if (isFollowing) {
    ...
    const popularityScore = bookmarkCount + likeCount * 3 + Math.min(viewCount / 10, 50);
    ...
    // 개인화 가중치 (태그 선호도)
    const tagScore = post.tags?.reduce((sum, postTag) => {
...
// 팔로잉 사용자들의 게시글 조회
const getFollowingPosts = async (userId, limit, cursor) => {
    const cursorCondition = buildCursorCondition(cursor);
    return await db_1.prisma.post.findMany({
        where: {
            AND: [
                cursorCondition,
                { deletedAt: null },
...
// 개인화 추천 게시글 조회
const getPersonalizedPosts = async (userId, limit, cursor, excludeIds = []) => {
    const cursorCondition = buildCursorCondition(cursor);
    return await db_1.prisma.post.findMany({
        where: {
            AND: [
...
// 비회원용 공개 게시글 조회
const getPublicPosts = async (limit, cursor) => {
    const cursorCondition = buildCursorCondition(cursor);
    return await db_1.prisma.post.findMany({
        where: {
            AND: [
...
const getHomeFeed = async (req, res, next) => {
    try {
        ...
        const { limit = 10, cursor, algorithmWeight = {
            following: 1.0,
            popular: 0.3,
            recent: 0.5,
            personalized: 0.7,
        ...
        const feedLimit = Math.min(Number.isFinite(parsedLimit) && parsedLimit > 0 ? parsedLimit : 10, 50);
        // 비회원과 회원 분기 처리
        if (!userId) {
            // 비회원인 경우: 공개 게시글만 제공 (hasMore 판별을 위해 feedLimit+1로 조회)
            const publicPosts = await getPublicPosts(feedLimit + 1, cursor);
            const feedPosts = publicPosts.map((post) => ({
                id: post.id,
                title: post.title,
                description: post.description || undefined,
                author: {
            ...
            const nextCursor = hasMore
                ? makeNextCursor(publicPosts[Math.min(publicPosts.length, feedLimit) - 1])
                : undefined;
            const response = {
                posts: postsOut,
                pagination: {
        ...
        // 0. 최근 24h 내 팔로잉 중 아직 노출되지 않은 게시글 선반영 (미확인 우선)
        const twentyFourHoursAgo = new Date(Date.now() - 24 * 60 * 60 * 1000);
        const unseenFollowingRecent = await db_1.prisma.post.findMany({
            where: {
                AND: [
            ...
            orderBy: [{ createdAt: "desc" }, { id: "desc" }],
            take: Math.ceil(feedLimit * 0.4),
        });
        // 1. 팔로잉 사용자들의 게시글 (우선순위 높음)
        const followingPosts = await getFollowingPosts(userId, Math.ceil(feedLimit * 0.6) * 2, // 후보 풀을 넉넉히 가져와 다양성 보장
        cursor);
        // 2. 개인화 추천 게시글
        const excludeIds = followingPosts.map((post) => post.id);
        const personalizedPosts = await getPersonalizedPosts(userId, Math.ceil(feedLimit * 0.4), // 내부에서 *2로 더 가져옴
        cursor, excludeIds);
        // 3. 사용자의 북마크 및 팔로잉 정보 조회
        const [userBookmarks, userFollowings] = await Promise.all([
            db_1.prisma.bookmark.findMany({
                where: { userId },
                select: { postId: true },
            }),
        ...
        // 4. 모든 게시글을 하나의 배열로 합치고 알고리즘 점수 계산 (미확인 우선 → 팔로잉 → 개인화, 중복 제거)
        const seenIds = new Set();
        const allPosts = [
            ...unseenFollowingRecent,
            ...followingPosts,
            ...personalizedPosts,
        ].filter((p) => (seenIds.has(p.id) ? false : (seenIds.add(p.id), true)));
        // 부가 신호 조회: 사용자별 노출/뷰, 저자 활동성(최근 7일 내 게시물 존재 여부)
        const candidateIds = allPosts.map((p) => p.id);
        const authorIds = Array.from(new Set(allPosts.map((p) => p.authorId)));
        const [myExposures, myViews, activeAuthors] = await Promise.all([
            db_1.prisma.postExposure.findMany({
                where: { userId, postId: { in: candidateIds } },
                select: { postId: true },
            }),
            db_1.prisma.postView.findMany({
                where: { userId, postId: { in: candidateIds } },
                select: { postId: true },
            }),
        ...
        const scoredPosts = allPosts.map((post) => {
            ...
            let score = calculatePostScore(post, userId, isFollowing, tagPreferences, algorithmWeight);
            // 미확인 팔로잉 우선
            if (unseenFollowingRecent.some((p) => p.id === post.id))
                score += 30;
            // 저활동 작성자 신선도(최근 7일 내 활동이 없는 저자)
            if (!activeAuthorSet.has(post.authorId))
                score += 10;
            // 스킵 패널티(노출 O, 뷰 X)
            if (exposedSet.has(post.id) && !viewedSet.has(post.id))
                score -= 20;
            // 시간대 보정(현재 시각±2h)
            const diffHours = Math.abs(new Date().getHours() - new Date(post.createdAt).getHours());
            if (diffHours <= 2)
                score += 5;
            return {
                ...post,
                algorithmScore: score,
                feedReason: isFollowing
                    ? "following"
        ...
        // 5. 점수순으로 정렬하고 다양성 보장 (같은 작성자 연속 방지)
        const sortedPosts = scoredPosts.sort((a, b) => b.algorithmScore - a.algorithmScore);
        const diversifiedPosts = [];
        const recentAuthorIds = new Set();
        const authorLastIndex = new Map();
        for (const post of sortedPosts) {
            const lastIndex = authorLastIndex.get(post.authorId);
            // 같은 작성자의 게시글이 너무 가까이 있지 않도록 체크
            if (!lastIndex || diversifiedPosts.length - lastIndex >= 3) {
                ...
                if (diversifiedPosts.length >= feedLimit)
                    break;
            }
        }
        // 5.5. 회원의 좋아요 상태 조회
        ...
        {
            const ids = diversifiedPosts.map((p) => p.id);
            if (ids.length > 0) {
                const likes = await db_1.prisma.postLike.findMany({
        ...
        // 6. 응답 데이터 구성
        const feedPosts = diversifiedPosts.map((post) => ({
            id: post.id,
            title: post.title,
            description: post.description || undefined,
            author: {
        ...
        // 7. 페이지네이션 정보
        // 최종 가드: 혹시라도 상위 로직 변경으로 개수가 늘어날 경우를 대비해 slice
        const postsOut = feedPosts.slice(0, feedLimit);
        const hasMore = sortedPosts.length > postsOut.length;
        const nextCursor = hasMore
            ? makeNextCursor(diversifiedPosts[Math.min(diversifiedPosts.length, feedLimit) - 1])
            : undefined;
        // 8. 알고리즘 정보
        const algorithmInfo = {
            ...
            recentCount: feedPosts.filter((p) => {
                const hoursAgo = (Date.now() - new Date(p.createdAt).getTime()) / (1000 * 60 * 60);
                return hoursAgo < 24;
            }).length,
        };
        const response = {
        ...
        try {
            if (userId) {
                await db_1.prisma.postExposure.createMany({
                    data: postsOut.map((p) => ({ userId, postId: p.id })),
                    skipDuplicates: true,
                });
            }
        }
    ...
    try {
        ...
        if (userId) {
            const [bookmark, follow, like] = await Promise.all([
                db_1.prisma.bookmark.findUnique({
                    where: {
                        userId_postId: {
                            userId,
                            postId: post.id,
                ...
                db_1.prisma.postLike.findUnique({
                    where: {
                        userId_postId: {
                            userId,
                            postId: post.id,
        ...
        try {
            if (userId) {
                await db_1.prisma.postView.create({ data: { userId, postId: post.id } });
            }
        }
        catch { }
        const response = {
...
const getComments = async (req, res, next) => {
    try {
        ...
        const comments = await db_1.prisma.comment.findMany({
            where: {
                AND: [cursorCond, { postId: postIdNum }, { deletedAt: null }],
            },
            include: {
                author: { select: { id: true, name: true, profileImageUrl: true } },
                mentions: {
...
const createComment = async (req, res, next) => {
    try {
        ...
        if (Array.isArray(mentions) && mentions.length > 0) {
            mentionIds = Array.from(new Set(mentions.map((m) => m.id).filter(Boolean)));
        }
        else {
            const atNames = Array.from(new Set((content.match(/@([A-Za-z0-9_가-힣]+)/g) || []).map((m) => m.slice(1))));
            if (atNames.length > 0) {
